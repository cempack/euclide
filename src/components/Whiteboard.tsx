...
export default function Whiteboard({ tabId, fileId }: { tabId: string; fileId?: number }) {
  ...
  return (
    <div className="h-full flex flex-col eu-drag">
      ...
      <div ref={wrapRef} className="flex-1 min-h-0 bg-[#f8f7f4] border-t border-hairline overflow-auto eu-drag" onWheel={e=>{if((e.ctrlKey||e.metaKey)&&wrapRef.current){e.preventDefault();const f=e.deltaY<0?1.1:0.9;setZoom(z=>Math.max(0.25,Math.min(4,z*f)));}}}>
